...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, src_mask: Optional[Tensor] = None,
        ...
        '''
        if self.norm_first:
            q = q + self._sa_block(self.norm1(q), self.norm1(k), self.norm1(v), src_mask, src_key_padding_mask)
            q = q + self._ff_block(self.norm2(q))
        else:
            q = self.norm1(q + self._sa_block(q, k, v, src_mask, src_key_padding_mask))
